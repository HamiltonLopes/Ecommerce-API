...
class Payment {
    static async compra(body, paymentCompany) {
        ...
        if (paymentCompany == 'Cielo') {
            ...
            data = await axios.post('https://api.pagar.me/1/transactions', exampleBody);
    ...
    static async consulta(paymentId) {
        ...
        //     data = await response.json();
        // }else{
          let  data = await axios.get(`https://api.pagar.me/1/transactions/${paymentId}`,{params: {"api_key":process.env.API_KEY}});
        // }
        console.log(data);
        return false;
        // return data;
